...
ECs  = W_matrix(env.data = maizeWTH, var.id = c("FRUE",'SPV',"n","T2M_MAX"),statistic = 'mean')
env_kernel(env.data = ECs)
env_kernel(env.data = ECs,gaussian=TRUE)
```

## get_kernel: preparing kernels for running predictive models
